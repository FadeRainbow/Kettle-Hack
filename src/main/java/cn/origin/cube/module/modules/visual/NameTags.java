...
public class NameTags extends Module {
    ...
    DoubleSetting radius = registerSetting("Radius", 1.0, 1.0, 5.0).booleanDisVisible(background);
...
//            }
            if(cFont.getValue()) {
                Cube.fontManager.CustomFont.drawStringWithShadow(e.getName() + " " + (Object) TextFormatting.GREEN + health + (heart.getValue() ? "\u2764" : "%"), 0 - this.getcenter(e.getName() + " " + (Object) TextFormatting.GREEN + health + "%") + cfontOffsetX.getValue(), 2+cfontOffsetY.getValue(), -1);
            }else{
    ...
    public void drawOutlineRoundedRect(int x, int y, int right, int bottom, int color, int radius, double distance, double distanceScale) {
        ...
        double divisor = MathUtil.lerp(MathHelper.clamp(distance * (sizel.maxValue + 1 - sizel.getValue()), 1, 90), MathHelper.clamp((sizel.maxValue + 1 - sizel.getValue()) * 2, 1, distance), sizel.getValue());
         ...
         drawPolygonOutline(0, 90, (int) (360 / divisor), x, y, radius, 1f, color);
        drawPolygonOutline(90, 180, (int) (360 / divisor), right-radius*2, y, radius, 1f, color);
        drawPolygonOutline(180, 270, (int) (360 / divisor), right-radius*2, bottom-radius*2, radius, 1f, color);
        drawPolygonOutline(270, 360, (int) (360 / divisor), x , bottom-radius*2, radius, 1f, color);
        GlStateManager.pushMatrix();
        GlStateManager.enableBlend();
        GlStateManager.disableDepth();
        GlStateManager.tryBlendFuncSeparate(770, 771, 0, 1);
